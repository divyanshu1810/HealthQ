...
export default function ChatApp() {
  ...
  // State to track browser support (initialized as true to prevent hydration mismatch)
  const [isBrowserSupportAvailable, setIsBrowserSupportAvailable] = useState(true);

  // Speech recognition with react-speech-recognition
  const {
    transcript,
    listening,
    resetTranscript,
  ...
  // Check browser support for speech recognition after component mounts
  useEffect(() => {
    setIsBrowserSupportAvailable(browserSupportsSpeechRecognition);
  }, [browserSupportsSpeechRecognition]);
  ...
  const handleAsk = async () => {
    ...
    resetTranscript(); // Also reset the transcript
          ...
          <div className="flex w-full gap-2">
            <div className="relative flex-grow">
              <input 
                ...
                className={`border-2 border-black bg-white text-black p-2 w-full rounded-lg pr-10 ${listening ? 'border-blue-500' : ''}`}
                disabled={loading}
              />
              <button
